...
#with the governance index from world bank
rm(list=ls())
...
#add the WB governance effectiveness indicator into the data (for comparison)
govWB_data<-as.data.frame(read_excel(paste0(input_fold,"wgidataset.xlsx")))
govWB_data<-govWB_data[which(govWB_data$indicator=='ge'),]
years<-c('2019','2020','2021','2022','2023')
govWB_data<-govWB_data[which(govWB_data$year %in% years),]
yearly_data<-matrix(NA,length(unique(govWB_data$code)),5)
country_order<-unique(govWB_data$code)
for(i in years){
  dat<-govWB_data[which(govWB_data$year==i),c(2,6)]
  dat$estimate[which(dat$estimate=='..')]<-NA
  yearly_data[,years==i]<-as.numeric(dat[match(country_order,dat$code),2])
}
yearly_data<-data.frame(iso3=country_order,estimate=rowMeans(yearly_data))
#fixing country codes to be consistent with UNHCR
the_na<-country_order[which(is.na(match(yearly_data$iso3,world_shape$iso3)))]
code_UNHCR<-c('ADO','COD','JEY','KSV','LIE','MCO','ANT','ROU','SMR','TLS','PSE')
for (i in 1:length(the_na)){
  yearly_data$iso3[(yearly_data$iso3==the_na[i])]<-code_UNHCR[i]
}
#reverse to account for worst as highest value
yearly_data$misgovWB<- -yearly_data[,2]
bti_match<-world_shape$misgov_ind[match(yearly_data$iso3,world_shape$iso3)]
yearly_data$misgovWB[is.na(bti_match)]<-NA
yearly_data$misgovWB<-(yearly_data$misgovWB-min(yearly_data$misgovWB,na.rm=T))/(max(yearly_data$misgovWB,na.rm=T)-min(yearly_data$misgovWB,na.rm=T))

world_shape$govWB<-yearly_data$estimate[match(world_shape$iso3,yearly_data$iso3)]
world_shape$misgovWB<-yearly_data$misgovWB[match(world_shape$iso3,yearly_data$iso3)]

rast_base<-composite_hist_index
values(rast_base)<-NA
BgovWB_rast<-rasterize(world_shape,rast_base,field='misgovWB')
#build the new risk index of hazard and misgovernance 
riskWB_hist<-(composite_hist_index+BgovWB_rast)/2
#categorize the new risk maps
brk  <- seq(0,1,0.2)
riskWB_hist_class <- raster::cut(riskWB_hist, breaks=brk) 
...
riskWB_hist_class_tbl<-overlayRast(riskWB_hist_class)
Visu_raster_discrete_with_points("./Plots/with_FDP/climate_govWB_hist_with_FDP.tiff","",'Risk class',my_pals,c('1','2','3','4','5'),c('1','2','3','4','5'),c('low','moderate','high','severe','extreme'),riskWB_hist_class_tbl,PoC_points,world_shape)
...
for (c in 1:nrow(world_shape)){
  ...
  hazgovWB_hist[c]<-exact_extract(riskWB_hist,world_shape[c,],'mean')
  hist_cl<-cut(hazgovWB_hist[c],breaks=seq(0,1,0.2),label=1:5)
  hist_cl<-as.numeric(levels(hist_cl))[hist_cl]
  hazgovWB_hist_class[c]<-hist_cl
...
world_shape$hazgovWB_hist<-hazgovWB_hist
world_shape$hazgovWB_hist_class<-hazgovWB_hist_class
world_shape$region<-region

world_shape$hazgovWB_hist[is.na(world_shape$misgovWB)]<-NA
world_shape$hazgovWB_hist_class[is.na(world_shape$misgovWB)]<-NA
...
world_data<-st_drop_geometry(world_shape)
for (c in (1:nrow(PoC_data))){
  the_ind<-which(world_data$iso3==PoC_data$country_code[c])
  if (length(the_ind)!=1){
     
    if (PoC_data$country_code[c]=='PRT'){
      the_ind<-the_ind[2]
      the_hazards[c,]<-as.numeric(world_data[the_ind,(ncol(world_data)-4):(ncol(world_data)-1)])
      the_region<-c(the_region,world_data$region[the_ind])
    }
    ...
    if (PoC_data$country_code[c]=='AB9'){
      the_ind<-which(world_data$gis_name=='Abyei')
      the_hazards[c,]<-as.numeric(world_data[the_ind,(ncol(world_data)-4):(ncol(world_data)-1)])
      the_region<-c(the_region,world_data$region[the_ind])
    }
...
names(PoC_data)[(ncol(PoC_data)-3):ncol(PoC_data)]<-c('haz_hist','haz_class_hist','riskWB_value_hist','riskWB_class_hist')

#Plot FDP numbers per region
...
#plot FDP number per risk class within bars
#####
...
for (the_reg in 1:length(all_region)){
  ...
  for (cla in hazard_class){
    region_ind<-the_reg_data[which(the_reg_data$riskWB_class_hist==cla),]
...
tiff(paste0('./Plots/with_FDP/FDP_number_govWB_per_region_with_bar_hist.tiff'),units="in", width=17, height=9, res=500,compression = 'lzw')
the_bar_plot<-ggplot()+
...
#######
###plot the misgovernance

spect_pal<-rev(brewer.pal(5,'Spectral'))

world_shape$misgovWB<-world_data$misgovWB
misgovWB_class<-cut(world_shape$misgovWB,breaks=seq(0,1,0.2),labels=1:5)
misgovWB_class<-as.numeric(levels(misgovWB_class))[misgovWB_class] 
world_shape$misgovWB_class<-misgovWB_class

tiff("./Plots/misgovWB_classes.tiff", units="in", width=10, height=6, res=500,compression = 'lzw')
plot(world_shape['misgovWB_class'],pal=spect_pal,breaks=0:5)
dev.off()
...
tiff("./Plots/misgovWB.tiff", units="in", width=10, height=6, res=500,compression = 'lzw')
ggplot()+
  geom_sf(data=world_shape,aes(fill=misgovWB),lwd=0.5)+
...
world_shape$haz_hist_class<-world_data$haz_hist_class
world_shape$haz_hist<-world_data$haz_hist
world_shape$hazgovWB_hist<-world_data$hazgovWB_hist

world_shape$hist_diff<-world_shape$haz_hist-world_shape$hazgovWB_hist
...
tiff("./Plots/govWB_improvement_hist.tiff", units="in", width=10, height=6, res=500,compression = 'lzw')
ggplot()+
